...
SETTINGS = {
	...
	// Template Url
	templateUrl: '1O9Mxtb5TpoRSDTinJnKPXjr2H2AytmJ2ZYLNm5pT1AE',

	// Set name spreadsheet
	spreadsheetName: 'Facture Data',

	//Set name document
	documentName: 'Facture Template',
	...
	webhooks = {
		test: "https://discord.com/api/webhooks/857343336489943051/vZ8DarZD9M-E80lBI5bIn1eVRLao8JkuWoTrv5FkPKJgEM2M9R-O26w1i95zIYwbKNma"
	}
};
...
function onOpen() {
	...
	SpreadsheetApp.getUi()
		.createMenu('Facturation')
		.addItem('Emettre une facture', 'sendInvoice')
		.addToUi();
}
...
function createSystem() {
	...
	try {
		...
		if (!systemCreated.getValue()) {
			...
			showUiDialog('Warnning', 'La configuration à déjà été faite!');
			return;
		}
		...
		var invoiceFolder = DriveApp.createFolder('Dossier de facturation');
		var folder = invoiceFolder.createFolder('Factures');
		...
		// End process
		showUiDialog('Success', 'La configuration est prête');

		return true;
	} catch (e) {

		// Show the error
		showUiDialog("Quelque chose s'est mal déroulé", e.message)
...
function sendInvoice() {
	...
	try {
		...
		if (!control) {
			showUiDialog('Warnning', 'Vous devez executer "Configuration auto" dans la feuille Instructions');
			return;
		}
				...
				for (var j = 0; j < sheetValues[i].length; j++) {
					...
					if (key === "date") {
						...
						if (key.indexOf("price") > -1 || key === "discount" || key.indexOf("total") > -1) {
							...
							replace('%' + key + '%', "Tel: " + values, docBody); // Replace the phone
						} else {
...
function showDialog() {
	var html = HtmlService.createHtmlOutputFromFile('iframe.html')
		.setWidth(200)
		.setHeight(150)
	SpreadsheetApp.getUi().showModalDialog(html, 'Configuration en cours..')
}
...
function sendMessage(pdfUrl, channel) {
	if (SETTINGS.webhooks.hasOwnProperty(channel))
		var url = SETTINGS.webhooks[channel];
	else {
	...
	var payload = JSON.stringify({
		...
		embeds: [
			{
			  title: "Nouvelle facture disponible ici",
			  description: "Cette facture à été générée automatiquement (merci :wink: <@295531682495201282>)",
			  url: pdfUrl,
			  color: 7069566
			}
		  ]
